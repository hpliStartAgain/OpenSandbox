...
def _apply_secure_access_env_overrides(config: AppConfig) -> None:
    ...
    signing key. Both must be set together, and environment variables win over
    any ``[ingress.secure_access]`` block from the config file.
    """
    ...
    for pair in keys_env.split(","):
        ...
        if not sep or not key_id:
            raise ValueError(
                f"{SECURE_ACCESS_KEYS_ENV_VAR} entries must be in key_id=base64 form, got {pair!r}"
            )
