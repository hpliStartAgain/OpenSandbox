...
The server proxy supports HTTP and WebSocket traffic and is also integrated with optional renew-on-access behavior. The HTTP proxy is transparent: it preserves the backend status code, media type, and response body instead of normalizing backend `3xx`, `4xx`, or `5xx` responses into OpenSandbox errors. Because the sandbox application controls those responses, the generated OpenAPI contract describes them with a `default` response rather than enumerating every possible backend status. It strips hop-by-hop headers and the backend `Server` header while preserving an origin `Date`; the server adds a current `Date` only when the response does not already contain one. A root-relative `Location` value that starts with a single `/` is rebased under the same sandbox proxy route, while absolute URLs, network-path references (`//host/path`), and ordinary path-relative values are forwarded unchanged.
